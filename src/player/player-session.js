...
import { LoopMode, SessionState } from "./core/playback-constants.js";
...
export class PlayerSession extends EventEmitter {
  ...
  async stop({ textChannelId, controllerUserId, interactionToken } = {}) {
    return this.#runSerial(() =>
      this.#stop({ textChannelId, controllerUserId, interactionToken }),
    );
  }
  ...
  #bindPlayerEvents() {
    ...
    this.player.on("error", (error) => {
      ...
      if (isStall) {
        this.#startNextTrack({ forceAdvance: false, retryCurrent: true }).catch(
          () => null,
        );
      } else {
