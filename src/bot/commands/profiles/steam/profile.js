...
async function getSteamProfileBackground(steam64, existingHtml = null) {
  try {
    ...
    const bgDiv = $(
      ".has_profile_background, .full_width_background, .no_header.profile_page",
    );
    if (bgDiv.length > 0) {
    ...
    const regex =
      /https?:\/\/[^"'\s]+(steamcommunity|fastly\.steamstatic)\.com\/(community_assets|public)\/images\/items\/[^"'\s]+\.(jpg|png|webp)/gi;
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("steamid")
        .setDescription(
          "輸入 Steam64 ID、自定義網址名稱或完整網址（非檔案名稱/profile name）",
        )
        .setRequired(true),
    ),
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const levelRes = await fetch(
        `https://api.steampowered.com/IPlayerService/GetSteamLevel/v1/?key=${STEAM_KEY}&steamid=${steam64}`,
      );
      const level = (await levelRes.json()).response?.player_level || "未知";

      const ownedRes = await fetch(
        `https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key=${STEAM_KEY}&steamid=${steam64}&include_appinfo=true&include_played_free_games=true`,
      );
      ...
      const totalHours = Math.floor(
        allGames.reduce((sum, g) => sum + (g.playtime_forever || 0), 0) / 60,
      );

      const recentRes = await fetch(
        `https://api.steampowered.com/IPlayerService/GetRecentlyPlayedGames/v1/?key=${STEAM_KEY}&steamid=${steam64}&count=4`,
      );
      ...
      if (player.timecreated) {
        ...
        const years = Math.floor(
          (Date.now() - createdDate) / (1000 * 60 * 60 * 24 * 365),
        );
      ...
      let _friendCount = 0;
      let _showcaseTitle = "";
      let _showcaseImage = null;

      try {
        const profileRes = await fetch(
          `https://steamcommunity.com/profiles/${steam64}?l=english`,
          {
            headers: {
              "User-Agent":
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36",
        ...
        const frameSrc =
          $(
            ".profile_avatar_frame picture img, .profile_avatar_frame source",
          ).attr("srcset") || $(".profile_avatar_frame img").attr("src");
        ...
        badgeCount =
          parseInt($(".profile_count_link_total").first().text().trim()) || 0;
        ...
        if (customization.length > 0) {
          _showcaseTitle = customization
            .find(".profile_customization_header")
            .text()
            .trim();
          const img = customization.find("img").first();
          if (img.length > 0) {
            _showcaseImage = img.attr("src");
          }
        }
      } catch (e) {
        console.log("[Steam] 額外資料抓取失敗（使用預設）", e);
      }
      ...
      if (featuredBadge) {
        try {
          ...
          ctx.fillStyle = "#0f1724";
          ctx.fillRect(rightX, contentY + 42, 50, 50);
          ctx.fillStyle = "#ffffff";
          ctx.font = "bold 15px 'Microsoft YaHei'";
          ctx.fillText("Unknown Badge", rightX + 58, contentY + 65);
          ctx.fillStyle = "#b0bec5";
          ctx.font = "12px 'Microsoft YaHei'";
          ctx.fillText("N/A", rightX + 58, contentY + 82);
        }
      }
      ...
      for (let i = 0; i < Math.min(badgePreviews.length, 3); i++) {
        try {
          ...
          ctx.fillStyle = "#0f1724";
          ctx.fillRect(badgeX, contentY + 100, 40, 40);
          badgeX += 46;
        }
      }
      ...
      ctx.fillText(
        `擁有 ${ownedCount} 款遊戲 • 總遊玩 ${totalHours} 小時`,
        contentX,
        contentY + 195,
      );
      ...
      for (let i = 0; i < Math.min(recentGames.length, 4); i++) {
        ...
        try {
          ...
          ctx.fillStyle = "#0f1724";
          ctx.fillRect(contentX, y, 105, 40);
        }
        ctx.fillStyle = "#e0e0e0";
        ctx.font = "13px 'Microsoft YaHei'";
        ctx.fillText(
          `${game.name} — ${(game.playtime_2weeks / 60).toFixed(1)} 小時`,
          contentX + 115,
          y + 24,
        );
        y += 48;
      ...
      for (let i = 0; i < Math.min(mostPlayed.length, 5); i++) {
        ...
        try {
          ...
          ctx.fillStyle = "#0f1724";
          ctx.fillRect(contentX + 700, y, 105, 40);
        }
        ctx.fillStyle = "#e0e0e0";
        ctx.font = "13px 'Microsoft YaHei'";
        ctx.fillText(
          `${game.name} — ${(game.playtime_forever / 60).toFixed(1)} 小時`,
          contentX + 815,
          y + 24,
        );
        y += 48;
