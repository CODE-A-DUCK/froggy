...
const LOOP_SEQUENCE = { 關閉: "重播一次", 重播一次: "單曲循環", 單曲循環: "關閉" };
...
function applyOptimisticState(components, action) {
  const visit = (c) => {
    ...
    if (typeof c.content === "string") c.content = updateContent(c.content, action);
    const id = c.custom_id;
    if (id === "MusicButtonControlPause" && action === "pause")
      Object.assign(c, { custom_id: "MusicButtonControlResume", label: "繼續", style: 3 });
    else if (id === "MusicButtonControlResume" && action === "resume")
      Object.assign(c, { custom_id: "MusicButtonControlPause", label: "暫停", style: 1 });
