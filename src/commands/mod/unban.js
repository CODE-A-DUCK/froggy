...
export const unbanCommand = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt.setName("user_id").setDescription("要解除封鎖的成員 ID").setRequired(true),
    )
    .addStringOption((opt) =>
  ...
  async execute(interaction, context) {
    ...
    try {
      ...
      if (!member.permissions.has(PermissionFlagsBits.BanMembers)) {
        return interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 你沒有封鎖成員權限" });
      }

      const botMember = interaction.guild.members.me;
      if (!botMember.permissions.has(PermissionFlagsBits.BanMembers)) {
        return interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 我沒有權限解除封鎖" });
      }
      ...
      await interaction.editReply({
        content: "<:errorwarningline:1510529314515320944> | 解除封鎖時發生錯誤，請確認目標成員的 ID 是否正確，並且我是否有足夠的權限",
      });
    }
  },
};