  ...
  async execute(interaction, context) {
    ...
    try {
      ...
      if (!member.permissions.has(PermissionFlagsBits.ModerateMembers)) {
        return interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 你沒有管理成員的權限" });
      }

      const botMember = interaction.guild.members.me;
      if (!botMember.permissions.has(PermissionFlagsBits.ModerateMembers)) {
        return interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 我沒有管理成員的權限" });
      }
      ...
      if (!targetMember.isCommunicationDisabled()) {
        return interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 該成員目前並沒有被禁言" });
      }
      ...
      if (
        ...
        return interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 該成員的權限高於或等於你，無法解除其禁言" });
      }
      ...
      await interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 解除禁言時發生錯誤" });
    }
  },
};