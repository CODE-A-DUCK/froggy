  ...
  async execute(interaction, context) {
    ...
    try {
      ...
      if (!member.permissions.has(PermissionFlagsBits.ManageChannels)) {
        return interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 你沒有管理頻道的權限" });
      }

      const botMember = interaction.guild.members.me;
      if (!botMember.permissions.has(PermissionFlagsBits.ManageChannels)) {
        return interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 我沒有管理頻道的權限" });
      }
      ...
      if (targetChannel.type !== ChannelType.GuildText) {
        return interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 只能在文字頻道設定慢速模式" });
      }
      ...
      await interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 設定慢速模式時發生錯誤。" });
    }
  },
};