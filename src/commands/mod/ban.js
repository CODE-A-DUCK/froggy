...
export const banCommand = {
  ...
  data: new SlashCommandBuilder()
    .setName("ban")
    .setDescription("將指定成員封鎖，除非管理員手動解除或使用/unban指令，否則將無法從新加入伺服器")
    .addUserOption((opt) =>
      opt
        .setName("user")
  ...
  async execute(interaction, context) {
    ...
    try {
      ...
      if (targetMember) {
        if (
          ...
          return interaction.editReply({
            content: "<:errorwarningline:1510529314515320944> | 你無法封鎖權限高於或等於你的成員",
          });
        }

        if (targetMember.roles.highest.position >= botMember.roles.highest.position) {
          return interaction.editReply({
            content: "<:errorwarningline:1510529314515320944> | 我無法封鎖該成員，該成員的權限高於或等於我",
          });
        }
      }
      ...
      await interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 封鎖目標成員時發生錯誤" });
    }
  },
};
