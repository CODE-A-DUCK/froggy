...
const bgPath = join(__dirname, "../../assist/bg_hoi.jpg");
...
export const hoi4profileCommand = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("steamid").setDescription("輸入 Steam64 ID 或網址").setRequired(true),
    ),
  ...
  async execute(interaction, context) {
    ...
    try {
      ...
      if (cleanInput.startsWith("profiles/")) steam64 = cleanInput.split("/")[1];
      else if (cleanInput.startsWith("id/")) {
        const vanity = cleanInput.split("/")[1];
        const res = await fetch(`https://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=${STEAM_KEY}&vanityurl=${vanity}`);
        const data = await res.json();
        if (data.response?.success !== 1) return interaction.editReply("<:errorwarningline:1510529314515320944> | 找不到這位 Steam 玩家");
        steam64 = data.response.steamid;
      } else if (!/^\d{17}$/.test(cleanInput)) {
        const res = await fetch(`https://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=${STEAM_KEY}&vanityurl=${cleanInput}`);
        const data = await res.json();
        if (data.response?.success !== 1) return interaction.editReply("<:errorwarningline:1510529314515320944> | 找不到這位 Steam 玩家");
        steam64 = data.response.steamid;
      }
      ...
      const ownedRes = await fetch(`https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key=${STEAM_KEY}&steamid=${steam64}&include_appinfo=true`);
      const hoi4Game = ((await ownedRes.json()).response?.games || []).find(g => g.appid === 394360);
      const playtimeHours = hoi4Game ? Math.floor((hoi4Game.playtime_forever || 0) / 60) : 0;

      let achieved = 0, totalAch = 0;
      try {
        const achRes = await fetch(`https://api.steampowered.com/ISteamUserStats/GetPlayerAchievements/v0001/?appid=394360&key=${STEAM_KEY}&steamid=${steam64}`);
      ...
      //置中卡片
      const cardX = 80, cardY = 50, cardW = 1240, cardH = 420, radius = 32;
      ...
      ctx.fillText("Froggy DC 機器人 • Hearts of Iron IV", leftX, cardY + cardH - 35);

      const buffer = canvas.toBuffer("image/png");
      const attachment = new AttachmentBuilder(buffer, { name: "hoi4-profile.png" });
      await interaction.editReply({ files: [attachment] });

    } catch (error) {
      console.error("[HOI4Profile] Error:", error);
      await interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 資料取得失敗" });
    }
  },
};