...
export const youtubeCommand = {
  ...
  async execute(interaction, context) {
    ...
    if (!YOUTUBE_API_KEY) {
      return interaction.editReply("我似乎尚未設定 YouTube API Key，請聯絡開發者");
    }
    ...
    try {
      ...
      if (!data.items || data.items.length === 0) {
        return interaction.editReply("<:errorwarningline:1510529314515320944> | 找不到這個 YouTube 頻道，請確認 ID 或 @handle 是否正確");
      }
      ...
      const attachment = new AttachmentBuilder(buffer, { name: "youtube-profile.png" });
      await interaction.editReply({ files: [attachment] });
    } catch (error) {
      console.error("[Command:YouTube] Error:", error);
      await interaction.editReply({ content: "<:errorwarningline:1510529314515320944> | 該頻道可能已設為不公開，或暫時無法取得資料" });
    }
  },
};