...
import { clearCommands } from "../commands/index.js";
...
function parseArgs() {
  ...
  if (args.includes("--global")) {
    ...
    console.info("Usage:");
    console.info("  node src/deploy/clear.js --global");
    console.info("  node src/deploy/clear.js --guild <guild_id>");
    process.exit(1);
  }
...
async function run() {
  try {
    ...
    console.info(
      `Clearing commands for application ${applicationId} in ${scope}...`,
    );
    ...
    console.info("Commands cleared successfully!");
    process.exit(0);
  } catch (error) {
    console.error("Command clearing failed:", error);
    process.exit(1);
  }
}
